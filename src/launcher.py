    ...
    if len(component) == 0:
        ...
        try:
            ...
            process: subprocess.Popen = subprocess.Popen(
                f"/bifrost/{COMPONENT['name']}/pipeline.py -pre {str(args.pre_script)} -per {str(args.per_sample_script)} -post {str(args.post_script)} -meta {str(args.run_metadata)} -reads {str(args.reads_folder)} {optional_values}",
                stdout=sys.stdout,
                stderr=sys.stderr,
                shell=True
            )
